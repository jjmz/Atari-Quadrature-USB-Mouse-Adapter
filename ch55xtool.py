...
def __write_data_ch55x_v23(dev, chk_sum, chip_id, payload):
    payload = payload 
    file_length = len(payload)
    ...
    while curr_addr < file_length:
        ...
        __WRITE_DF_V2 = WRITE_DF_V2
        __WRITE_DF_V2[1] = pkt_length 
        __WRITE_DF_V2[2] = curr_addr % 256
        __WRITE_DF_V2[3] = (curr_addr >> 8) % 256
        __WRITE_DF_V2 = __WRITE_DF_V2 + payload[curr_addr:curr_addr+pkt_length]
        
        dev.write(EP_OUT_ADDR, __WRITE_DF_V2)
        ret = dev.read(EP_IN_ADDR, 6, USB_MAX_TIMEOUT)
...
def __main():
    ...
    parser.add_argument('-d', '--data', type=str, default='', help="The target dataflash file to be flashed.")
    ...
    if args.file != '':
        ...
        if args.data != '':
            payload = list(open(args.file, 'rb').read())
            ret = __write_data_ch55x_v23(dev, chk_sum, chip_id, payload)
            if ret is None:
                sys.exit('Failed to write Data to CH55x.')
